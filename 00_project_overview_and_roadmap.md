...
monitor:9090 Prometheus -> app:8080/metrics                    # реализовано: product metrics + HTTP/API request metrics
...
Backend/application node. Сейчас Python-приложение работает как `misis-digital-student-support-api`: `/v1/health`, `/v1/support-model`, `/v1/tickets`, `/v1/tickets/all`, `/v1/tickets/<id>`, `/v1/tickets/<id>/status`, `/metrics`. Заявки хранятся в `/opt/app/tickets.json`; product logs пишутся в `/var/log/app/app.log`; app logs отправляются в Loki через Promtail; product metrics и HTTP/API request metrics собираются Prometheus.
...
```text
Последний завершенный этап: Этап 14. HTTP request metrics, error-rate alerts и latency.
Текущий следующий этап: Этап 15. Dockerization.
Далее: Dockerization, PostgreSQL, DB observability, Telegram bot, hardening, Ansible automation v2, final README/demo.
```
...
```text
Формальная готовность по расширенному roadmap: 14/21 основных этапов завершены ≈ 67%.
Готовность core infrastructure lab: 100% по этапам 1–10.
Admin/Ansible foundation: 100%.
Product model v2: 100%.
Инженерная готовность по новому production-like scope: 72–77%.
Демонстрационная готовность текущего core-проекта: 94–96%.
Финальная демонстрационная готовность с DB/Docker/Bot/Ansible v2: 56–64%.
```
...
| 15. Dockerization | следующий этап | 0–5% | Docker пока не внедрен; следующий production-like этап — контейнеризация backend-а без изменения внешнего flow. |
